...
# Spreading Udaciousness：需要多长时间才可以将本课程招募满学员
...
def lucky_search(index, ranks, keyword):
    pages = lookup(index, keyword)#从index中找到与keyword相关的网页
    if not pages:#如果不存在，返回None
        return None
    best_page = pages[0]#如果存在，即使说至少有一页了，命名为best_page
    for candidate in pages:
        if ranks[candidate] > ranks[best_page]:
            best_page = candidate
    return best_page#结束返回best_page
